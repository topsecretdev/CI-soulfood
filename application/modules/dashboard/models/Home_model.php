...
class Home_model extends CI_Model {
	...
	public function totalamount($dailySales=null)
	{
	    $amount=0.00;
        $todaysDate = date("Y-m-d"); # or any other date

        $today=date('Y-m-d');
        $query= $this->db->select('SUM(total_price) as amount')->from('booked_info');
        if($dailySales!=null){
            $query=$query->like('date_time',$todaysDate);
        }

            $query=$query->where('bookingstatus!=', 1)->get();
        $val=0;
        if ($query->num_rows() > 0) {
        ...
        $query= $this->db->select('SUM(totalamount) as amount')->from('customer_order');
        if($dailySales!=null){
            $query->where('order_date',$todaysDate);
        }
        $query=$query->get();
        if ($query->num_rows() > 0) {
            $val2=$query->row()->amount;
            $val=$val+$val2;
        }

        $query= $this->db->select('SUM(totalamount) as amount')->from('customer_order2');
    if($dailySales!=null){
    $query=$query->where('order_date',$todaysDate);
        }
        $query=$query->get();
        if ($query->num_rows() > 0) {
            $val3=$query->row()->amount;
            $val=$val+$val3;
        }
