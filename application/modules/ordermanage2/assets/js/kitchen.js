...
$( window ).load(function() {
  ...
  window.location.href = basicinfo.baseurl+"ordermanage2/order/allkitchen";
 }, 300000);
 setInterval(function(){ 
  load_unseen_notification(); 
 }, 700);
 setInterval(function(){ 
  window.location.href = basicinfo.baseurl+"ordermanage2/order/kitchen";
 }, 600000);
});
"use strict";
...
$('input[type="checkbox"]').click(function(){
           ...
           if(isaccept==1){
                $.ajax({
				type: "POST",
				url: basicinfo.baseurl+"ordermanage2/order/itemisready",
				data: dataString,
...
if(values==''){
	...
	return false;
			...
			$.ajax({
				type: "POST",
				url: basicinfo.baseurl+"ordermanage2/order/itemacepted",
				data: dataString,
				success: function(data){
					if(data==1){
	...
	function itemcancel(){
	  ...
	  $('body').on('click', '#itemcancel', function() {
			...
			$.ajax({
			type: "POST",
			url: basicinfo.baseurl+"ordermanage2/order/cancelitem",
			data: dataString,
			success: function(data){
...
function onprepare(ordid,kitid){
	var values = $('input[name="item'+ordid+kitid+'"]:checked').map(function() {
		...
		$.ajax({
			type: "POST",
			url: basicinfo.baseurl+"ordermanage2/order/markasdone",
			data: dataString,
			success: function(data){
...
function printtoken(ordid,kitid){
	var values = $('input[name="item'+ordid+kitid+'"]:checked').map(function() {
		...
		$.ajax({
			type: "POST",
			url: basicinfo.baseurl+"ordermanage2/order/printtoken",
			data: dataString,
			success: function(data){
				$("#kotenpr").html(data);
...
function oredrready(orderid){
	...
	var dataString = 'orderid='+orderid+'&csrf_test_name='+csrf;
	 $.ajax({
			type: "POST",
			url: basicinfo.baseurl+"ordermanage2/order/checkorder",
			data: dataString,
			success: function(data){
...
function oredrisready(orderid){
	...
	var dataString = 'orderid='+orderid+'&csrf_test_name='+csrf;
	 $.ajax({
			type: "POST",
			url: basicinfo.baseurl+"ordermanage2/order/orderisready",
			data: dataString,
			success: function(data){
 ...
 function load_unseen_notification()
 {
	 ...
	 var view=''
  $.ajax({
   url: basicinfo.baseurl+"ordermanage2/order/notification",
   method:"POST",
