...
function posupdatecart(id,pid,vid,qty,status){
		...
		new Audio(mysound + audio[0]).play();
		 ...
		 var dataString = "CartID="+id+"&qty="+qty+"&Udstatus="+status+'&pid='+pid+'&csrf_test_name='+csrf;
			 ...
			 /*	if(data == 'out_of_stock')
       		{
       			...
       			swal({
					...
					text: "Item max available qty is "+qty,
...
//Product search button js
	...
	$(document).on('click', '.aceptorcancel', function(){ 
		...
		if(productionsetting == 1){
		   ...
		   $.ajax({
					...
					url:baseurl+"ordermanage2/order/checkstock",
	...
	function adonseditinfo(id){
		...
		var myurl =baseurl+'ordermanage2/menu_addons/assignaddonsupdateinfo/'+id;
	...
	function editvarient(id){
		...
		var myurl =baseurl+'ordermanage2/item_food/updateintfrm/'+id;
	...
	function editavailable(id){
		...
		var myurl =baseurl+'ordermanage2/item_food/updateavailfrm/'+id;
	...
	$( "#item_list" ).autocomplete({ 
		...
		type: "POST",
			...
			url: baseurl+"ordermanage2/item_food/checkfood",
		...
		minLength:1,
			...
			select: function (event, ui){
				...
				var varientname=ui.item.variant2Name
