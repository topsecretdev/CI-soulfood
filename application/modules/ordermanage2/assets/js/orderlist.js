     ...
     var allordertable = $('#tallorder').DataTable({ 
		...
		"searching": true,
		  "processing": true,
				 "serverSide": true,
				 "ajax":{
					url :baseurl+"ordermanage2/order/allorderlist",
					type: "post",
					"data": function ( data ) {
...
function noteCheck(id){
	 ...
	 var myurl =baseurl+'ordermanage2/order/ajaxupdateoreder/';
	  ...
	  function showhide(id){
		   ...
		   var isVisible = $( '#item'+id ).is(".active");
			if (isVisible === true ){
				...
				$.ajax({
				type: "POST",
				url: baseurl+"ordermanage2/order/itemlist",
				data: dataString,
				success: function(data){
					$('#item'+id).html(data);
...
function getreport(){
    ...
    //     return false;
    // }
    var myurl = baseurl+"ordermanage2/order/allorderreportlist";

    $.ajax({
        type: "POST",
        url: myurl,
        data: {csrf_test_name: csrf, from_date: from_date, to_date: to_date,user_role:role},
        success: function(data) {
            debugger;
            var html="";
            var total=0;
            ...
            html+=`<tr style=" text-align: right;font-weight: bold;"><td colspan="100">Total: ${res.total}</td></tr>`;
