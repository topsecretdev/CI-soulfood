...
$(document).ready(function(){
 ...
 setInterval(function(){ 
  window.location.href = baseurl+"ordermanage2/order/counterboard";
 }, reloadtime);
});
"use strict";
function oredrready(orderid){
	...
	var dataString = 'orderid='+orderid+'&csrf_test_name='+csrf;
	 $.ajax({
			type: "POST",
			url: baseurl+"ordermanage2/order/checkorder",
			data: dataString,
...
function oredrisready(orderid){
	...
	var dataString = 'orderid='+orderid+'&csrf_test_name='+csrf;
	 $.ajax({
			type: "POST",
			url: baseurl+"ordermanage2/order/orderisready",
			data: dataString,
			success: function(data){
 ...
 function load_unseen_notification(view = ''){
	 var csrf = $('#csrf_token').val();
  $.ajax({
   url: urlajax+"ordermanage2/order/notification",
   method:"POST",
