<link rel="stylesheet" type="text/css" href="<?php echo base_url('application/modules/ordermanage2/assets/css/pos_token.css'); ?>">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('application/modules/ordermanage2/assets/css/pos_print.css'); ?>">
<?php defined('BASEPATH') OR exit('No direct script access allowed');?>
<div id="printableArea" class="bill2__container bill2-pos-mini__container width-font">
    <div class="pt-5">
        <div class="bill2-pos-mini__logo border" align="center"><img src="<?php echo base_url();?><?php echo $storeinfo->logo?>" class="img img-responsive" alt=""></div>
    </div>
    <div class="px-4">
        <h5 class="text-center mt-3 mb-0 text-bold"><?php echo display('bill2');?></h5>
        ...
        <div class="pb-3 border-bottom--dashed">
            <table class="w-100">
                ...
                <?php $this->load->model('ordermanage2/order_model',	'ordermodel');
				  ...
				  $totalamount=0;
					...
					?>
                    <tr>
                        <td class="p-0"><p class="mb-0"><?php echo $item->ProductName;?>-<?php echo $item->variant2Name;?></p><small class="mb-0 text-italic"><?php echo $item->price;?> x <?php echo $item->menuqty;?></small></td>
			...
			if(!empty($item->add_on_id)){
			 ...
			 $servicecharge=0; 
			 if(empty($bill2info)){ $servicecharge;}
			 else{$servicecharge=$bill2info->service_charge;}
			 
			 $sdr=0; 
			 if($storeinfo->service_chargeType==1){ 
			 $sdpr=$bill2info->service_charge*100/$bill2info->total_amount;
			 $sdr='('.round($sdpr).'%)';
			 } 
			 else{$sdr='('.$currency->curr_icon.')';}
			 
			  $discount=0; 
			 if(empty($bill2info)){ $discount;}
			 else{$discount=$bill2info->discount;}
			 
			 $discountpr=0; 
			 if($storeinfo->discount_type==1){ 
			 $dispr=$bill2info->discount*100/$bill2info->total_amount;
        ...
        <div class="border-bottom--dashed">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <p class="mb-0 text-note text-primary text-bold"><?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo $bill2info->total_amount;?> <?php if($currency->position==2){echo $currency->curr_icon;}?></p>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-2">
                <p class="mb-0 text-note text-primary"><?php echo display('discount')?></p>
                <p class="mb-0 text-note text-primary">-<?php if($currency->position==1){echo $currency->curr_icon;}?>  <?php $discount=0; if(empty($bill2info)){ echo $discount;} else{echo $discount=$bill2info->discount;} ?> <?php if($currency->position==2){echo $currency->curr_icon;}?></p>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-2">
                <p class="mb-0 text-note text-primary"><?php echo display('service_chrg')?>:</p>
                <p class="mb-0 text-note text-primary"><?php if($currency->position==1){echo $currency->curr_icon;}?><?php $sdcharge=0; if(empty($bill2info)){ echo $sdcharge;} else{echo $sdcharge=$bill2info->service_charge;} ?><?php if($currency->position==2){echo $currency->curr_icon;}?></p>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-2">
                <p class="mb-0 text-note text-primary"><?php echo display('vat_tax')?>(<?php echo $storeinfo->vat;?>%):</p>
                <p class="mb-0 text-note text-primary"><?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo $calvat=$bill2info->VAT; ?> <?php if($currency->position==2){echo $currency->curr_icon;}?></p>
            </div>
        </div>
        <div class="border-bottom--dashed">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <p class="mb-0 text-note text-primary text-bold"><?php echo display('grand_total')?>:</p>
                <p class="mb-0 text-note text-primary text-bold"><?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo $bill2info->bill2_amount;?> <?php if($currency->position==2){echo $currency->curr_icon;}?></p>
            </div>
            <?php 
        ...
        <div class="">
            <div class="d-flex justify-content-between">
                ...
                <?php if($bill2info->bill2_status==1){?><p class="mb-0 text-note"><?php if($currency->position==1){echo $currency->curr_icon;}?>  <?php echo $customepaid; ?> <?php if($currency->position==2){echo $currency->curr_icon;}?></p>
