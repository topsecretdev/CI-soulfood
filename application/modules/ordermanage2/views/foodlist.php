...
<table class="table table-bordered table-hover bg-white" id="purchaseTable">
								...
								foreach($itemlist as $item){
                    				...
                    				<input name="itemname" type="hidden" id="itemname_<?php echo $i?>" value="<?php echo $item->ProductName;  if(!empty($item->itemnotes)){ echo " -".$item->itemnotes;}?>" />
                                        ...
                                        <td>
                                        <input name="sizeid" type="hidden" id="sizeid_<?php echo $i?>" value="<?php echo $item->variant2id;?>" />
                                        <input name="size" type="hidden" value="<?php echo $item->variant2Name;?>" id="size_<?php echo $i;?>" />
                                        ...
                                        <?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo $item->price;?> <?php if($currency->position==2){echo $currency->curr_icon;}?>
                                            ...
                                            <td><?php if($getadons==1){?>
												 <a class="btn btn-success" onclick="addonsitem(<?php echo $item->ProductsID;?>,<?php echo $item->variant2id;?>)" data-toggle="modal" data-target="#edit" data-dismiss="modal"><?php echo display('add_to_cart')?></a>
												<?php }
											else{
											?>
