							...
							foreach($alliteminfo as $single){
								...
								$allisexit=$this->db->select('tbl_kitchen2_order2.*')->from('tbl_kitchen2_order2')->where('orderid',$orderinfo->order_id)->where('kitchenid',$kitchenid)->where('itemid',$single->menu_id)->where('varient',$single->variant2id)->get()->num_rows();
								if($allisexit>0){
								$allchecked2.="1,";
								}
							else{
                                    ...
                                    <div class="food_select" id="acceptitem<?php echo $orderinfo->order_id.$kitchenid;?>">
                                    	<?php 
										 ...
										 foreach($iteminfo as $item){
											 $l++;
											 $ischecked=$this->db->select('tbl_kitchen2_order2.*')->from('tbl_kitchen2_order2')->where('orderid',$orderinfo->order_id)->where('kitchenid',$kitchenid)->where('itemid',$item->menu_id)->where('varient',$item->variant2id)->get()->num_rows();?>
