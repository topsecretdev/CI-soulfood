        ...
        <?php } 
		$datafield='amount='.round($orderinfo->totalamount).'|currencyCode=978|merchantId='.$paymentinfo->marchantid.'|normalReturnUrl='.base_url().'ordermanage2/order/successful/'.$orderinfo->order_id.'|transactionReference='.$orderinfo->order_id.'|keyVersion='.$paymentinfo->email.'';
		$secretKey=$paymentinfo->password;
		$seal=hash('sha256', $datafield.$secretKey);
		?>
<input type="hidden" name="Data" value="amount=<?php echo round($orderinfo->totalamount);?>|currencyCode=978|merchantId=<?php echo $paymentinfo->marchantid;?>|normalReturnUrl=<?php echo base_url();?>ordermanage2/order/successful/<?php echo $orderinfo->order_id;?>|transactionReference=<?php echo $orderinfo->order_id;?>|keyVersion=<?php echo $paymentinfo->email;?>">
