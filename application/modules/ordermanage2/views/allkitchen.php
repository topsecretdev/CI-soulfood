...
<div class="row">
                   <div class="panel">
                     <div class="panel-body">
                         <div class="text-right"><a class="display-none" id="fullscreen" href="#"><i class="pe-7s-expand1"></i></a><a href="<?php echo base_url();?>ordermanage2/order/allkitchen" class="btn btn-primary btn-md"><i class="fa fa-load-circle" aria-hidden="true"></i>
                    ...
                    <div class="grid-sizer col-vxs-12 col-xs-6 col-md-4 col-lg-3 col-xlg-4"></div>
					    <?php if(!empty($kitchenorderinfo)){
							...
							foreach($alliteminfo as $single){
								...
								$allisexit=$this->db->select('tbl_kitchen2_order2.*')->from('tbl_kitchen2_order2')->where('orderid',$orderinfo->order_id)->where('kitchenid',$orderinfo->kitchenid)->where('itemid',$single->menu_id)->where('varient',$single->variant2id)->get()->num_rows();
								if($allisexit>0){
								$allchecked2.="1,";
								}
							else{
                                    ...
                                    <div class="food_select" id="acceptitem<?php echo $orderinfo->order_id.$orderinfo->kitchenid;?>">
                                    	<?php 
										 ...
										 foreach($iteminfo as $item){
											 $l++;
											 $ischecked=$this->db->select('tbl_kitchen2_order2.*')->from('tbl_kitchen2_order2')->where('orderid',$orderinfo->order_id)->where('kitchenid',$orderinfo->kitchenid)->where('itemid',$item->menu_id)->where('varient',$item->variant2id)->get()->num_rows();?>
                                        <div class="single_item">
                                            <div class="align-center justify-between item-dv">
                                                ...
                                                <label for='chkbox-<?php echo $l.$item->kitchenid.$orderinfo->order_id;?>'>
                                                   ...
                                                   <div>
                                                      ...
                                                      <?php if(!empty($item->varientid)){?><span class="item-span"><?php echo $item->variant2Name;?></span><?php } ?>
                                                   </div>
                                                </label>
											...
											foreach($addons as $addonsid){
												...
												$adonsinfo=$this->ordermodel->read('*', 'add_ons2', array('add_on_id' => $addonsid));
                                        ...
                                        <div class="single_item bgkitchen">
                                            <div class="align-center justify-between item-dv">
                                                 <div>
                                                     ...
                                                     <span class="item-span"><?php echo $item->variant2Name;?></span>
                                                   </div>
...
<div class="modal fade modal-warning" id="posprint" role="dialog">
        ...
        <script src="<?php echo base_url('application/modules/ordermanage2/assets/js/kitchen.js'); ?>" type="text/javascript"></script>
