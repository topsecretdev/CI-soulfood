...
class Item_food extends MX_Controller {
...

    {
        ...
        $config["base_url"] = base_url('ordermanage2/item_food/index');
...

    {
	  ...
	  $this->form_validation->set_rules('foodname', display('item_name2')  ,'required|max_length[100]');

	  }

	  else{

		   $this->form_validation->set_rules('foodname', display('item_name2')  ,'required|is_unique[item_foods2.ProductName]|max_length[100]');
	  ...
	  /****************image Upload*************/
	    ...
	    $config['upload_path']          = 'application/modules/ordermanage2/assets/images/';
	    ...
	    $big='application/modules/ordermanage2/assets/images/big/'.$fdata['file_name'];

		$medium='application/modules/ordermanage2/assets/images/medium/'.$fdata['file_name'];

		$small='application/modules/ordermanage2/assets/images/small/'.$fdata['file_name'];

		$img = 'application/modules/ordermanage2/assets/images/'.$fdata['file_name'];
	  ...
	  $logData = array(
	   ...
	   'action_page'         => "Add Drink",

	   'action_done'     	 => "Insert Data", 

	   'remarks'             => "New Drink Added",
	  ...
	  $taxsettings = $this->taxchecking();
		...
		$this->logs_model->log_recorded($logData);
		 ...
		 redirect('ordermanage2/item_food/create');
		...
		$this->permission->method('ordermanage2','update')->redirect();
	  ...
	  $taxsettings = $this->taxchecking();
		...
		redirect("ordermanage2/item_food/create/".$postData['ProductsID']);
	  ...
	  $data['taxitems'] = $this->taxchecking();
	   ...
	   if(!empty($id)) {
		...
		$data['title'] = display('update_fooditem2');
...

    {
        ...
        $this->permission->module('ordermanage2','delete')->redirect();
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		redirect('ordermanage2/item_food/index');
		...
		$data['intinfo']="";
			...
			if ($this->form_validation->run()) { 
				...
				if ($this->fooditem_model->addsupplier($postData)) { 
					...
					redirect('ordermanage2/item_food/bulkfoodupload');
...

    {
        ...
        $data['title']    = display('variant2_list');
        ...
        $config["base_url"] = base_url('ordermanage2/item_food/foodvarientlist');
		...
		$data['title'] = display('variant2_edit');
...

    {
	  ...
	  #-------------------------------#
		...
		$this->form_validation->set_rules('foodid',display('item_name2')  ,'required');
	  ...
	  $data['varient']   = (Object) $postData = [
	   ...
	   'variant2id'          => $this->input->post('variant2id'),

	   'menuid' 	        => $this->input->post('foodid',true),

	   'variant2Name' 	 	=> $this->input->post('varientname',true),
	  ...
	  if ($this->form_validation->run()) { 
	   ...
	   if (empty($this->input->post('variant2id'))) {
	 ...
	 $logData = [
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		if ($this->foodvarient_model->create($postData)) { 
		 ...
		 redirect('ordermanage2/item_food/foodvarientlist');
   ...
   public function updateintfrm($id){
	   ...
	   $data['currency']=$this->fooditem_model->currencysetting($settinginfo->currency);
		...
		$this->load->view('ordermanage2/varientedit', $data);
...

    {
        ...
        $data['title']    = display('food_availablelist2');
        ...
        $config["base_url"] = base_url('ordermanage2/item_food/availablelist');
	 ...
	 $logData = [
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		if ($this->foodavailable_model->create($postData)) { 
		 ...
		 redirect('ordermanage2/item_food/availablelist');
   ...
   public function updateavailfrm($id){
		...
		$this->load->view('ordermanage2/availabledit', $data);
...

    {
	  ...
	  $img = $this->fileupload->do_upload('./application/modules/ordermanage2/assets/images/','picture');
	 ...
	 $logData = array(
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		if ($this->todaymenu_model->create($postData)) { 
		 ...
		 redirect('ordermanage2/item_food/todaymenutype');
   ...
   public function updatemenufrm($id){
		...
		$this->load->view('ordermanage2/mtypeedit', $data);
...

    {
	  ...
	  if($totalitem<2){
		   ...
		   redirect("ordermanage2/item_food/addgroupfood");
	  ...
	  $taxsettings = $this->taxchecking();
		...
		redirect("ordermanage2/item_food/addgroupfood/".$postData['ProductsID']);
	...
	public function checkfood(){
		...
		foreach ($product_info as $value) {
			...
			$json_product[] = array('label'=>$value['ProductName'].'_'.$value['variant2Name'],'value'=>$value['ProductsID'],"varientid"=>$value['variant2id'],"variant2Name"=>$value['variant2Name'],"ProductName"=>$value['ProductName'],"price"=>$value['price']);
        ...
        $data['title']    = display('food_stock2');
        ...
        $config["base_url"] = base_url('ordermanage2/item_food/food_stock');
        ...
        $data["fooditemsliststock"] = $this->fooditem_model->read_fooditemstock($config["per_page"], $page);
