...
class Restauranttable extends MX_Controller {
    ...
    public function index($id = null)
    {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('ordermanage2/restauranttable/index');
	 ...
	 $logData = [
	  ...
	  $lastid=$this->restable_model->create($postData);
		...
		'tableid'         => $lastid,
		 ...
		 redirect('ordermanage2/table-list');
		} else {
   ...
   public function updateintfrm($id){
        ...
        $data['module'] = "ordermanage2";
        $data['page']   = "tableedit";
		$this->load->view('ordermanage2/tableedit', $data);
	   }
	...
	public function updatesetting(){
		 ...
		 $tableexist=$this->db->select("*")->from('table_setting2')->where('tableid',$ids)->get()->row();
				if(empty($tableexist)) { 
				$rowdata=array(
				'tableid'         => $ids,
				'iconpos'		  => $style
	...
	public function showfile(){
			    $data['module'] = "ordermanage2";
				$data['page']   = "alllayout";
				$this->load->view('ordermanage2/alllayout', $data);
	 ...
	 $logData = array(
	  ...
	  $lastid=$this->restable_model->createfloor($postData);
		if($lastid) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage2/restauranttable/floorlist');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage2/restauranttable/floorlist");
	 ...
	 $logData = array(
	  ...
	  $lastid=$this->restable_model->createroom($postData);
		if($lastid) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage2/restauranttable/roomlist');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage2/restauranttable/roomlist");
...
public function updateroomfrm($id){
        ...
        $data['module'] = "ordermanage2";
        $data['page']   = "roomedit";
		$this->load->view('ordermanage2/roomedit', $data);
	   }
