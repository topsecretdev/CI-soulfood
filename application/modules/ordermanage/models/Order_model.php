...
class Order_model extends CI_Model
{
	...
	{
		$this->db->select('customer_order.*,CONCAT_WS(" ",customerinfo.firstname,customerinfo.lastname) as customer_name,customerinfo.customerid,customer_type.customer_type,CONCAT_WS(" ", employee_history.first_name,employee_history.last_name) AS fullname,rest_table.tablename,sec_role_tbl.role_name, variant.variantName as variantName, variant.price as price, order_menu.menuqty as menuqty, COALESCE(variant.price, 0) * COALESCE(order_menu.menuqty, 0) as total');
		$this->db->from('customer_order');
		$this->db->join('customerinfo', 'customer_order.customer_id=customerinfo.customerid', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
		$this->db->join('employee_history', 'customer_order.waiter_id=employee_history.emp_his_id', 'left');
		$this->db->join('sec_user_access_tbl', 'customer_order.user_id=sec_user_access_tbl.fk_user_id', 'left');
		...
		$this->db->join('variant', 'variant.variantid = order_menu.varientid');
		$this->db->order_by('customer_order.order_id', 'DESC');
		$this->db->limit($limit, $start);
		$query = $this->db->get();
		$list = $query->result();
	...
	{
		$this->db->select('order_menu.*,bill.total_amount');
		$this->db->from('order_menu');
		$this->db->join('bill', 'order_menu.order_id=bill.order_id', 'inner');



		$this->db->where('order_menu.row_id', $id);
	...
	private function get_allorder_report_query($role = 1, $from = null, $to = null)
	{
		$column_order = array(null, 'customer_order.saleinvoice', 'customerinfo.customerid', 'customer_type.customer_type', 'CONCAT_WS(" ", employee_history.first_name,employee_history.last_name)', 'rest_table.tablename', 'customer_order.order_date', 'customer_order.totalamount',); //set column field database for datatable orderable
		$column_search = array('customer_order.saleinvoice', 'customerinfo.customerid', 'customer_type.customer_type', 'CONCAT_WS(" ", employee_history.first_name,employee_history.last_name)', 'rest_table.tablename', 'customer_order.order_date', 'customer_order.totalamount,sec_role_tbl.role_name', 'sec_role_tbl.role_id'); //set column field database for datatable searchable
		$order = array('customer_order.order_id' => 'asc');
		$this->db->select('customer_order.*,CONCAT_WS(" ",customerinfo.firstname,customerinfo.lastname) as customer_name,customerinfo.customerid,customer_type.customer_type,CONCAT_WS(" ", employee_history.first_name,employee_history.last_name) AS fullname,rest_table.tablename,sec_role_tbl.role_name,sec_role_tbl.role_id');
		$this->db->from('customer_order');
		$this->db->join('customerinfo', 'customer_order.customer_id=customerinfo.customerid', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
		$this->db->join('employee_history', 'customer_order.waiter_id=employee_history.emp_his_id', 'left');
		...
		$this->db->join('sec_role_tbl', 'sec_role_tbl.role_id=sec_user_access_tbl.fk_role_id', 'left');
		$this->db->join('rest_table', 'customer_order.table_no=rest_table.tableid', 'left');
		// $this->db->where('sec_role_tbl.role_id',14);
		$this->db->order_by('customer_order.order_id', 'DESC');
